...
export function useLand(id: string | undefined) {
  ...
  const { data, isLoading, error } = useReadContract({
    abi: LAND_REGISTRY_ABI, address: CONTRACT_ADDRESS,
    functionName: "get_land", args: ok ? [BigInt(id!)] : undefined, enabled: ok,
  });
...
export function useTokenUri(id: string | undefined) {
  ...
  const { data, isLoading } = useReadContract({
    abi: LAND_REGISTRY_ABI, address: CONTRACT_ADDRESS,
    functionName: "token_uri", args: ok ? [BigInt(id!)] : undefined, enabled: ok,
  });
...
export function useHistory(id: string | undefined) {
  ...
  const { data, isLoading } = useReadContract({
    abi: LAND_REGISTRY_ABI, address: CONTRACT_ADDRESS,
    functionName: "get_transfer_history", args: ok ? [BigInt(id!)] : undefined, enabled: ok,
  });
  const history: TransferHistoryEntry[] = data ? parseHistory(data as any[]) : [];
...
export function useUserLands(address: string | undefined) {
  ...
  const { data, isLoading } = useReadContract({
    abi: LAND_REGISTRY_ABI, address: CONTRACT_ADDRESS,
    functionName: "get_user_lands", args: ok ? [address!] : undefined, enabled: ok,
  });
...
export function useTotalLands() {
  const { data, isLoading } = useReadContract({
    abi: LAND_REGISTRY_ABI, address: CONTRACT_ADDRESS,
    functionName: "get_total_lands", args: [], enabled: true,
  });
