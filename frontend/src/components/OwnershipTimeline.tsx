...
export function OwnershipTimeline({
  ...
  if (!history.length)
    return <p className="text-zinc-600 text-sm py-2">No history found.</p>;
    ...
    <ol className="relative border-l border-zinc-800 ml-3">
      {history.map((e, i) => (
        <li key={i} className="ml-6 mb-5 last:mb-0">
          <div
            className={`absolute -left-1.5 w-3 h-3 rounded-full border-2 ${i === history.length - 1 ? "bg-emerald-500 border-emerald-400" : "bg-zinc-700 border-zinc-600"}`}
          />
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {i === history.length - 1 && history.length > 1 && (
